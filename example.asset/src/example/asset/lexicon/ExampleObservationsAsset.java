...
public class ExampleObservationsAsset implements AssetProxy
{
    ...
    private void createTestObservations() throws ValidationFailedException
    {
        ...
        for (int i=0; i < Math.max(classSize, modalitySize); i++)
        {
            ...
            classificationSensingObs.setObservedTimestamp(System.currentTimeMillis() - TimeUnit.SECONDS.toMillis(1));
        ...
        Observation cbrneTriggerObs = new Observation().withSensorId("cbrne-trigger").withCbrneTrigger(cbrneTrigger);
        ...
        final Power power = new Power()
             ...
             .withSource(new PowerSource(
                            new ConfidenceFactor().withValue(0.50).withScore(
                                    ...
                                    "Appliance A did X"),
                         new PowerSource(new ConfidenceFactor().withValue(0.35), "Appliance A did Y"));
    ...
    public Observation onCaptureData() throws AssetException
    {
        ...
        // Offset the observed time so it is easily distinguishable from the created time, in particular when viewing
        // from the web GUI
